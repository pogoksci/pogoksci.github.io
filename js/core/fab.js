...
function toggleMenu(force) {
    ...
    const fab = document.getElementById("fab-button");
    if (fab) {
        // Assuming FAB has an icon element. If setMenu set it, it likely does.
        // If setVisibility set it, it might differ.
        // We only toggle icon if we are in 'Menu Mode' (detected by onclick handler or state?)
        // Simple heuristic: If menu is visible, show Close.
        const icon = fab.querySelector('.material-symbols-outlined');
        if (icon && fab.dataset.mode === 'menu') {
            // Store original icon if needed? Or just hardcode 'menu'/'close'
            icon.textContent = next ? "close" : "menu";
        }
    }
}
